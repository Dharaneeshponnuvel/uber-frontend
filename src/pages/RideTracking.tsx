...
import { MapPin, Clock, Car, Phone, Star, User } from 'lucide-react';
  ...
  useEffect(() => {
    if (socket) {
      socket.on('ride-accepted', (data) => {
        setCurrentRide((prev) =>
          prev ? { ...prev, status: 'accepted', driver: data.driver } : null
        );
      });

      socket.on('ride-completed', (data) => {
        setCurrentRide((prev) =>
          prev ? { ...prev, status: 'completed', final_fare: data.finalFare } : null
        );
        setShowRating(true);
      });
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/book-ride" className="btn-primary">
            Book a New Ride
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          ...
          <div
            className={`inline-flex items-center space-x-2 px-4 py-2 rounded-full ${statusInfo.color}`}
          >
            {statusInfo.icon}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
              ...
              <div className="flex space-x-3">
                ...
                <button onClick={handleRatingSubmit} className="flex-1 btn-primary">
                  Submit Rating
                </button>
              </div>
            </div>
